...
double K0 = 0; // Initial Kinetic Energy
double K = 0; // Kinetic Energy
double U; //Potential Energy
double H = 0; // Total Energy
...
int static count = 0, count2 = 0;
double dx_max=-1000, dx_min = 1000, dy_max=-1000, dy_min = 1000;
...
void systemInit(struct particle *atoms){
	...
	double lsp = L/((int)(sqrt(N))-1); // Lattice Spacing
	double U1, U2, norVar;
...
double calcuForces(struct particle *atoms){
	...
	for(i=0; i<N-1; i++){
		for(j=i+1; j<N; j++){
			...
			dx = atoms[i].rx-atoms[j].rx;
			dy = atoms[i].ry-atoms[j].ry;
			// dz = atoms[i].rz-atoms[j].rz;
			//printf("%f %f \n", dx, dy);
...
double motionInteg2(struct particle *atoms){
	int i;
	double flag = U;
	K = 0;
	
	for(i=0; i<N; i++){
		atoms[i].rx += atoms[i].vx*dt + 0.5*atoms[i].ax*dt*dt;
		atoms[i].ry += atoms[i].vy*dt + 0.5*atoms[i].ay*dt*dt;
	}
